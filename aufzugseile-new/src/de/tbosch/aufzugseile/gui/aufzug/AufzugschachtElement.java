...
import de.tbosch.seile.commons.elemente.EndElement;
 ...
 */
public class AufzugschachtElement extends JComponent implements Element {
	...
	/** The seil element. */
	private Vector<AufzugschachtSeilElement> seilElementVector = new Vector<AufzugschachtSeilElement>();
	 ...
	 * 
	 * @param scale The scale of width and heigth
	 * @param iconpath The path of the icon
	 * @param height Height
	 * @param aufzugschacht The elevator itself
	 * @param width Width
	 * @param seilx Relative x-coordinates of rope-clipping-point
	 * @param yscale The scale of y-coordinates
	 * @param seily Relative y-coordinates of rope-clipping-point
	 * @param xscale The scale of x-coordinates
	 * @param y y-position
	 * @param x x-position
	 */
	public AufzugschachtElement(int x, int y, int width, int height, int seilx, int seily, double scale, double xscale, double yscale, String iconpath, Aufzugschacht aufzugschacht) {
	...
	private void mouseClickedElement(MouseEvent evt) {
		...
		// create a popup-menu
		if (evt.getButton() == GUICommonsConstants.RIGHT_MOUSE_BUTTON && !aufzugschacht.mainFrameIsAnyButtonSelected) {
			...
			parameter2te = new JMenuItem(new AbstractAction("Parameter (zweite Umschlingung)", new ImageIcon(Helper.getFileURL(GUICommonsConstants.MODIFY_ICON))) {
				public void actionPerformed(ActionEvent e) {
					popupModify2te();
				}
			});
			JMenuItem deactivate = null;
			deactivate = new JMenuItem(new AbstractAction("Deaktivieren", new ImageIcon(Helper.getFileURL(GUICommonsConstants.DEACTIVATE_ICON))) {
				public void actionPerformed(ActionEvent e) {
					popupDeactivate();
				}
			});
			JMenuItem activate = null;
			activate = new JMenuItem(new AbstractAction("Aktivieren", new ImageIcon(Helper.getFileURL(GUICommonsConstants.ACTIVATE_ICON))) {
				public void actionPerformed(ActionEvent e) {
					popupActivate();
				}
			});
			JMenuItem seilAnders = null;
			seilAnders = new JMenuItem(new AbstractAction("Seilverlauf "+SystemConstants.AE_K+"ndern", new ImageIcon(Helper.getFileURL(GUICommonsConstants.CHANGE_ICON))) {
				public void actionPerformed(ActionEvent e) {
					popupChangeSeil();
				}
			});
			...
			if (!(this instanceof Seilaufhaenger)) {
				...
				if (this instanceof Umlenkrolle) {
					Rolle rolle = (Rolle)this;
					if (rolle.isActivated()) popup.add(deactivate);
					else popup.add(activate);
				}
			}
			...
			if (this instanceof Rolle) {
				Rolle rolle = (Rolle)this;
				if (!rolle.isDoppelteUmschlingung() 
						&& rolle.getSeilElement(true) != null
						&& rolle.getSeilElement(true).getPrev() != null
						&& rolle.getSeilElement(true).getPrev().getElement() instanceof Rolle
						&& rolle.getSeilElement(true).getNext() != null
		...
		else aufzugschacht.mouseClickedArea(evt);
	}
	...
	private void popupChangeSeil() {
		((AufzugschachtRolle)this).changeSeil();
	...
	private boolean isDoubleClickedElement(MouseEvent evt) {
		if (evt.getButton() == GUICommonsConstants.LEFT_MOUSE_BUTTON) {
	...
	private void mouseDraggedElement(MouseEvent evt) {
		...
		Point xy;
		if (this instanceof Seilaufhaenger) xy = getDraggingXYAufhaenger(ox, oy, dx, dy);
		else xy = getDraggingXY(ox, oy, dx, dy);
		int x = (int) xy.getX();
		int y = (int) xy.getY();
		...
		if (this instanceof AufzugschachtEndElement) {
			AufzugschachtEndElement endElement = (AufzugschachtEndElement) this;
			if (endElement.getUmlenkrolle() != null) {
				cy = cy - ((AufzugschachtUmlenkrolle)endElement.getUmlenkrolle()).getHeight();
				ch = ch + ((AufzugschachtUmlenkrolle)endElement.getUmlenkrolle()).getHeight();
			}
			if (endElement.getDoppelUmlenkrolle() != null) {
				cy = cy - ((AufzugschachtDoppelUmlenkrolle)endElement.getDoppelUmlenkrolle()).getHeight();
				ch = ch + ((AufzugschachtDoppelUmlenkrolle)endElement.getDoppelUmlenkrolle()).getHeight();
			}
			if (endElement.getSeilaufhaenger() != null) {
				cy = cy - ((AufzugschachtSeilaufhaenger)endElement.getSeilaufhaenger()).getHeight();
				ch = ch + ((AufzugschachtSeilaufhaenger)endElement.getSeilaufhaenger()).getHeight();
			}
		}
		Vector<Element> elementVector = aufzugschacht.getElementAt(x, cy, getWidth(), ch);
		boolean onlyElement = true;
		for (int i = 0; i < elementVector.size(); i++) {
			if (!elementVector.elementAt(i).equals(this)) if (this instanceof EndElement) {
				// the own clipped elements does not interfere the own move
				EndElement endElement = (EndElement) this;
				if (!elementVector.elementAt(i).equals(endElement.getSeilaufhaenger()) && !elementVector.elementAt(i).equals(endElement.getUmlenkrolle())
						&& !elementVector.get(i).equals(endElement.getDoppelUmlenkrolle())) onlyElement = false;
			}
			else onlyElement = false;
		}
		if (onlyElement) {
			...
			omy = my - dy;
			if (aufzugschacht.getSeil() != null) {
				((AufzugschachtSeil)aufzugschacht.getSeil()).refreshSeil();
				((AufzugschachtSeil)aufzugschacht.getSeil()).repaint();
			}
		}
	}
	 ...
	 * @param ox The old x-coordinate
	 * @param oy The old y-coordinate
	 * 
	...
	private Point getDraggingXY(int ox, int oy, int dx, int dy) {
		...
		if (this instanceof AufzugschachtEndElement) {
			AufzugschachtEndElement endElement = (AufzugschachtEndElement) this;
			if (endElement.getUmlenkrolle() != null) extraHeight = ((AufzugschachtUmlenkrolle)endElement.getUmlenkrolle()).getHeight();
			if (endElement.getSeilaufhaenger() != null) extraHeight = ((AufzugschachtSeilaufhaenger)endElement.getSeilaufhaenger()).getHeight();
		}

		// if the old coordinate + difference is not outside the area border it
		// is okay
		// otherwise the coordinates are set at the border
		if (ox + dx >= 1 && ox + dx <= aufzugschacht.getWidth() - 1 - getWidth()) x = ox + dx;
		else if (ox + dx < 1) x = 1;
		else x = aufzugschacht.getWidth() - 1 - getWidth();

		if (oy + dy - extraHeight >= 1 && oy + dy <= aufzugschacht.getHeight() - 1 - getHeight()) y = oy + dy;
		else if (oy + dy - extraHeight < 1) y = 1 + extraHeight;
		else y = aufzugschacht.getHeight() - 1 - getHeight();
	 ...
	 * @param yscale The scale of the y-coordinates
	 * @param xscale The scale of the x-coordinates
	 */
	public void setScale(double scale, double xscale, double yscale) {
		if (this.scale != scale || this.xscale != xscale || this.yscale != yscale) {
			...
			if (this instanceof AufzugschachtSeilaufhaenger) {
				AufzugschachtSeilaufhaenger aufhaenger = (AufzugschachtSeilaufhaenger) this;
				if (!aufhaenger.isUnderTreibscheibe()) {
					if (aufhaenger.getHimmel() == Seilaufhaenger.NORD) {
						if (x > aufzugschacht.getWidth() - width - 1) x = aufzugschacht.getWidth() - width - 1;
						if (x < 1) x = 1;
						y = 1;
					}
						...
						if (y > aufzugschacht.getHeight() - width - 1) y = aufzugschacht.getHeight() - height - 1;
						if (y < 1) y = 1;
						x = 1;
					}
					else {
	...
	public int getSeilX() {
		if (this instanceof AufzugschachtSeilaufhaenger) {
			if (((AufzugschachtSeilaufhaenger)this).getHangingElement() != null) {
				AufzugschachtElement hangingElement = (AufzugschachtElement)((AufzugschachtSeilaufhaenger)this).getHangingElement();
	...
	public void setSeilElement(SeilElement seilElement) {
		seilElementVector.add((AufzugschachtSeilElement)seilElement);
	}
	...
	private Point getDraggingXYAufhaenger(int ox, int oy, int dx, int dy) {
		...
		if (aufhaenger.getHimmel() == Seilaufhaenger.WEST || aufhaenger.getHimmel() == Seilaufhaenger.OST) {
			x = ox;
			if (oy + dy >= 0 && oy + dy <= aufzugschacht.getHeight() - 2 - getHeight() - 1) y = oy + dy;
			else if (oy + dy < 0) y = 1;
			else y = aufzugschacht.getHeight() - 2 - getHeight() - 2;
		}
		else {
			y = oy;
			if (ox + dx >= 0 && ox + dx <= aufzugschacht.getWidth() - 2 - getWidth() - 1) x = ox + dx;
			else if (ox + dx <= 0) x = 1;
			else x = aufzugschacht.getWidth() - 2 - getWidth() - 2;
		}
